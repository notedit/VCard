...
function LandingPage({ onStart }: { onStart: () => void }) {
  ...
  const showcaseDecks: Array<{ themeId: ThemeId; mode: Mode; label: string; meta: string; card: CardItem; index: number; total: number }> = [
    { themeId: 'mono', mode: 'html', label: 'Mono · 资讯拆解', meta: '文字版面 · 4:5', card: baseOutline[0], index: 0, total: 7 },
    { themeId: 'paper', mode: 'html', label: 'Paper · 杂志感', meta: '文字版面 · 1:1', card: baseOutline[3], index: 3, total: 7 },
    { themeId: 'graphite', mode: 'html', label: 'Graphite · 暗色信息板', meta: '文字版面 · 4:5', card: baseOutline[5], index: 5, total: 7 },
    { themeId: 'indigo', mode: 'image', label: 'Indigo · 视觉封面', meta: '整图封面 · 9:16', card: baseOutline[2], index: 2, total: 7 },
  ];

  const features: Array<{ icon: IconName; title: string; body: string }> = [
    { icon: 'cards', title: '大纲先行', body: '先把卡组拆成一张一张观点，再开始动版面。每张卡只承担一个论点，整套读下来不会乱。' },
    { icon: 'sparkles', title: '两种版面任选', body: '想稳就用文字版面，想吸睛就用整图封面。同一份大纲，两种风格随时切换。' },
    { icon: 'pin', title: '点哪张改哪张', body: '在工作台里点中任意卡片，AI 助手默认只改这一张，不会顺手把别的也改了。' },
    { icon: 'palette', title: '编辑级主题', body: '黑白灰为底，每个主题只配一个强调色。极简、杂志、暗色、彩色随时切换。' },
    { icon: 'layers', title: '版式自动匹配', body: '封面、要点、引言、数据、结尾——按内容自动分配，也可以手动改。' },
    { icon: 'download', title: '直接发到平台', body: '小红书、微信小绿书、即刻、公众号、X 的常用尺寸都已对齐，导出即发。' },
  ];

  const audiences: Array<{ kicker: string; title: string; body: string }> = [
    { kicker: '内容创作者', title: '把一篇稿件拆成一套可发的卡片', body: '从原始素材到 7-10 张可编辑卡，5 分钟出第一稿，再针对单卡逐张微调。' },
    { kicker: '知识工作者', title: '把研究、报告、长文沉淀成社媒资产', body: '用大纲控制论点，用主题控制气质。让非专业读者也能一口气读完。' },
    { kicker: '运营 / 教育', title: '稳定批量产出选题与连载', body: '同一套大纲反复换主题、调密度、改版式，把每周的内容生产做成流水线。' },
  ];
  ...
  return (
    <div className="landing-page">
      <header className="landing-nav">
        <div className="landing-nav-inner">
          ...
          <div className="landing-nav-actions">
            <button className="btn btn-primary btn-sm" type="button" onClick={onStart}>
              打开工作台
      ...
      <main>
        <section className="landing-hero">
          <div className="landing-hero-inner">
            <div className="landing-copy">
              <p className="landing-kicker">为创作者打造的卡片工作台</p>
              <h1>
                <span>一个想法</span>
                <em><b aria-hidden="true">→</b> 一套卡片</em>
              </h1>
              <p>
                AI 拆大纲 · 批量生成 · 对话微调，直接发。
              </p>
              ...
              <div className="landing-actions">
                <button className="btn btn-accent" type="button" onClick={onStart}>
                  开始创作
            ...
            <div className="landing-visual" aria-hidden="true">
              <div className="landing-card-collage">
                ...
                <article className="landing-hero-card">
                  ...
                  <p>VCARD · 封面</p>
                  <h2>OpenAI Codex:<br />重新定义编程的未来</h2>
        ...
        <section className="landing-modes" id="modes" aria-label="两种版面">
          <header className="landing-section-head">
            <p className="eyebrow">两种版面</p>
            <h2>同一份大纲，两种气质</h2>
            <p>讲清楚靠文字，靠氛围靠封面。两种版面共用同一套大纲、主题、版式，可随时切换。</p>
          </header>
          <div className="landing-modes-grid">
            <article className="landing-mode-card landing-mode-html">
              <div className="landing-mode-head">
                <span className="landing-mode-tag">
                  <Icon name="code" size={13} />
                  文字版面
                </span>
                <span className="landing-mode-time">出图快</span>
              </div>
              <h3>稳定可读的文字卡</h3>
              <p>排版可预测、字体可控、随时改文案。适合资讯拆解、观点输出、教程连载。</p>
              <ul className="landing-mode-points">
                <li>封面、要点、引言、数据、结尾五种版式自动匹配</li>
                <li>切换主题不重新生成，只换样式层</li>
                <li>每张卡都能在工作台里直接改文字</li>
              </ul>
              <div className="landing-mode-preview">
                <CardFrame card={baseOutline[0]} theme={getTheme('mono')} mode="html" size="4-5" density="standard" total={7} index={0} />
              </div>
            </article>

            <article className="landing-mode-card landing-mode-image">
              <div className="landing-mode-head">
                <span className="landing-mode-tag">
                  <Icon name="sparkles" size={13} />
                  整图封面
                </span>
                <span className="landing-mode-time">视觉优先</span>
              </div>
              <h3>有冲击力的封面图</h3>
              <p>整图生成，氛围饱满。适合热点封面、情绪表达、视觉化叙事。</p>
              <ul className="landing-mode-points">
                <li>编辑摄影、纪实、极简、产品、插画、抽象等多种风格</li>
                <li>异步生成不阻塞工作台，单张失败可单张重跑</li>
                <li>同一套大纲，与文字版面之间随时切换</li>
              </ul>
              <div className="landing-mode-preview">
                <CardFrame card={baseOutline[2]} theme={getTheme('indigo')} mode="image" size="4-5" density="standard" total={7} index={2} />
              </div>
            </article>
          </div>
        </section>

        <section className="landing-workflow" id="workflow" aria-label="5 步工作流">
          <header className="landing-section-head">
            <p className="eyebrow">5 步工作流</p>
            <h2>从主题到工作台，每一步都看得见</h2>
            <p>把"AI 突然给你一坨"拆成五个可控的小步。任意一步不满意，都能退回去改。</p>
          </header>
          <ol className="landing-steps">
            {[
              { num: '01', title: '输入', body: '给主题、定模式、选数量与尺寸。', icon: 'edit' as IconName },
              { num: '02', title: '大纲', body: '逐卡确认标题与要点，可增删改排。', icon: 'cards' as IconName },
              { num: '03', title: '风格', body: '挑主题、调文字密度、定版式或图像风格。', icon: 'palette' as IconName },
              { num: '04', title: '生成', body: '后台流式生成，进度实时反馈。', icon: 'sparkle' as IconName },
              { num: '05', title: '工作台', body: '缩略图 + 单卡预览 + 锁定式 AI 对话。', icon: 'layers' as IconName },
            ].map((step) => (
              <li key={step.num}>
        ...
        <section className="landing-showcase" id="showcase" aria-label="主题与版面范例">
          <header className="landing-section-head">
            <p className="eyebrow">主题 × 版面范例</p>
            <h2>同一份大纲，不同的气质</h2>
            <p>同一组内容，可以是极简、杂志、暗色信息板，也可以是有色彩的视觉封面——永远保持黑白灰为底、一个强调色定调。</p>
          </header>
          <div className="landing-showcase-row" role="list">
            {showcaseDecks.map((item) => {
              const theme = getTheme(item.themeId);
              return (
                <article className="landing-showcase-card" role="listitem" key={`${item.themeId}-${item.mode}`}>
                  <div className="landing-showcase-frame">
                    <CardFrame card={item.card} theme={theme} mode={item.mode} size="4-5" density="standard" total={item.total} index={item.index} />
                  </div>
        ...
        <section className="landing-features" id="features" aria-label="核心能力">
          <header className="landing-section-head">
            <p className="eyebrow">核心能力</p>
            <h2>一个人就能出整套卡片</h2>
            <p>不是 AI 玩具，也不是模版商城。是一台稳定、可控、随手能开工的卡片生产桌面。</p>
          </header>
        ...
        <section className="landing-audiences" aria-label="适用人群">
          <header className="landing-section-head">
            <p className="eyebrow">适用人群</p>
            <h2>给把内容当生产资料的人</h2>
          </header>
          <div className="landing-audiences-row">
            {audiences.map((item) => (
        ...
        <section className="landing-cta" aria-label="开始创作">
          <div className="landing-cta-inner">
            <p className="landing-kicker">Ready when you are</p>
            <h2>把今天的素材，做成今晚能发的卡片</h2>
            <p>本地保存草稿，随时回退上一步，单卡可控编辑。打开工作台后从「输入」开始。</p>
            <div className="landing-actions">
              <button className="btn btn-accent" type="button" onClick={onStart}>
                打开工作台
                <Icon name="arrowRight" size={15} />
              </button>
              <a className="btn btn-outline invert" href="#features">
                了解更多
              </a>
            </div>
          </div>
      ...
      <footer className="landing-footer">
        <div className="landing-footer-left">
          ...
          <span className="landing-footer-tag">Social card production desk</span>
        </div>
        <div className="landing-footer-meta">
          <span>© {new Date().getFullYear()} vCard</span>
          <span>·</span>
          <span>已适配 小红书 / 微信小绿书 / 即刻 / 公众号</span>
        </div>
      </footer>
    </div>
  );
